...
export function CreateUnitModal({ open, onClose, onAdd }: CreateUnitModalProps) {
  ...
  return createPortal(
    ...
    >
      ...
      >
        ...
        <h3 id="create-unit-title" className="pr-8 text-lg font-bold text-brand-primary">
          {t("dashboard.inventory.createItem.createUnitTitle")}
        </h3>

        <div className="mt-5">
          <label className="block text-sm font-medium text-brand-primary">
            {t("dashboard.inventory.createItem.unitNameLabel")}
          </label>
          <input
            type="text"
            value={name}
            ...
            placeholder={t("dashboard.inventory.createItem.unitNamePlaceholder")}
          ...
          {error && (
            <p className="mt-1 text-xs font-medium text-red-600">
              {t("dashboard.inventory.createItem.unitNameRequired")}
            </p>
          )}
        </div>
        ...
        <div className="mt-6 flex justify-end gap-2">
          ...
          >
            {t("dashboard.inventory.createItem.addUnit")}
          </button>
        </div>
      </div>
    </div>,
