...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    const { response, body } = await proxySalesBackend(
      request,
      `sales/organisations/${encodeURIComponent(organisationId)}/invoices/${encodeURIComponent(trimmedInvoiceId)}/send-email`,
      {
        method: "POST",
    ...
    if (!response.ok) {
      return NextResponse.json(
        { error: extractBackendError(body) ?? "Failed to send invoice email" },
    ...
    console.error("Sales invoice send-email error:", error);
