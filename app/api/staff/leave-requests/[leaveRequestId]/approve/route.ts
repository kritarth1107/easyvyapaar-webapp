...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    const { response, body } = await proxyStaffBackend(
      request,
      `staff/organisations/${encodeURIComponent(organisationId)}/leave-requests/${encodeURIComponent(leaveRequestId)}/approve`,
      {
        method: "POST",
    ...
    if (!response.ok) {
      return NextResponse.json(
        { error: extractBackendError(body) ?? "Failed to approve leave request" },
