...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    const backendUrl = new URL(
      `user/organisations/${encodeURIComponent(organisationId)}/members/invite/verify-otp`,
      apiBaseUrl,
    );
    ...
    console.error("Invite verify OTP error:", error);
    return NextResponse.json({ error: "Failed to verify invite OTP" }, { status: 500 });
  }
}
