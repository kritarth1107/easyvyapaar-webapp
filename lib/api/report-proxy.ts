...
export async function proxyReportBackend(
  request: Request,
  path: string,
  init?: RequestInit,
  ...
  if (!apiBaseUrl) {
    return {
      ...
      body: { error: "Reports service is not configured" },
    };
  }
  ...
  try {
    ...
    console.error("Reports backend request failed:", error);
    return {
      response: new Response(null, { status: 502 }),
      body: { error: "Unable to reach reports service" },
    };
  }
}
