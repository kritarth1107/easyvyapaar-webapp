...
export async function proxySalesBackend(
  request: Request,
  path: string,
  init?: RequestInit,
  ...
  if (!apiBaseUrl) {
    return {
      ...
      body: { error: "Sales service is not configured" },
    };
  }
  ...
  try {
    ...
    console.error("Sales backend request failed:", error);
    return {
      response: new Response(null, { status: 502 }),
      body: { error: "Unable to reach sales service" },
    };
  }
}
