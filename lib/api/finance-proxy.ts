...
export async function proxyFinanceBackend(
  request: Request,
  path: string,
  init?: RequestInit,
  ...
  if (!apiBaseUrl) {
    return {
      ...
      body: { error: "Finance service is not configured" },
    };
  }
  ...
  try {
    ...
    console.error("Finance backend request failed:", error);
    return {
      response: new Response(null, { status: 502 }),
      body: { error: "Unable to reach finance service" },
    };
  }
}
