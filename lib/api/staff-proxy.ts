...
export async function proxyStaffBackend(
  request: Request,
  path: string,
  init?: RequestInit,
  ...
  if (!apiBaseUrl) {
    return {
      ...
      body: { error: "Staff service is not configured" },
    };
  }
  ...
  try {
    ...
    console.error("Staff backend request failed:", error);
    return {
      response: new Response(null, { status: 502 }),
      body: { error: "Unable to reach staff service" },
    };
  }
}
